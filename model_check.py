from Dual_model.data_segmentation import SegmentationDataset
...
for inddir in tqdm(dir_list):
    if(int(inddir[0:3]) > 0 and int(inddir[0:3]) < 3):
        mask_directory = 'train_videos/'+inddir[0:3]+"/maskedImages/"
        image_directory = 'train_videos/'+inddir[0:3]+"/rawImages/"
        tempImage = sorted(list(paths.list_images(image_directory)))
        for tempIm in tempImage:
            imagePaths.append(tempIm)
        tempMask = sorted(list(paths.list_images(mask_directory)))
        for tempIm in tempMask:
            maskPaths.append(tempIm)
        # imagePaths.append(sorted(list(paths.list_images(image_directory))))
        # maskPaths.append(sorted(list(paths.list_images(mask_directory))))


print(config.IMAGE_DATASET_PATH)
